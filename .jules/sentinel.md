...
## 2026-07-05 - [IP Parser Bypass via Whitespace Padding]
**Vulnerability:** A vulnerability existed where an attacker could bypass IP tracking, rate-limiting, and blocklists by padding the X-Forwarded-For or remote address with leading or trailing whitespaces (e.g., ` 192.168.1.1 `) along with a scope id. If whitespace stripping happens before removing the scope id, trailing spaces will be left over. Python's `ipaddress` module strictly validates IP strings and rejects padded strings with a `ValueError`. This resulted in the IP being classified as invalid and circumventing tracking mechanisms dependent on `normalize_ip`.
**Learning:** Always carefully consider the order of operations when sanitizing input, particularly when dealing with split and strip operations.
**Prevention:** Always strip whitespace from strings *after* removing any potential scope identifiers (e.g., `raw_ip.split('%')[0].strip()`) before passing them to strict parsers like `ipaddress.ip_address`.

## 2026-07-18 - [Resource Exhaustion via Unrestricted Content-Length]
**Vulnerability:** A resource exhaustion Denial of Service (DoS) vulnerability existed where Flask/Werkzeug would begin to process request payloads before verifying if they exceeded `MAX_CONTENT_LENGTH`. An attacker could hold open connections while streaming massive payloads, exhausting server memory and connection pools.
**Learning:** While Werkzeug respects `MAX_CONTENT_LENGTH` during stream consumption, validating `request.content_length` proactively in an `@app.before_request` hook prevents the application from even starting to handle the payload, offering a stronger defense-in-depth.
**Prevention:** Always implement an explicit `content_length` check early in the request lifecycle (e.g., `if request.content_length > limit: return 413`) to explicitly block excessively large payloads before framework-level processing begins.
