...
## 2026-05-16 - [Rate Limiter Dictionary Eviction Vulnerability]
**Vulnerability:** The memory-bound rate limiter implemented bounded memory usage by removing the oldest dictionary entry when the max size was reached (`MAX_TRACKED_IPS`). However, it did not utilize a Least Recently Used (LRU) policy; active users were not moved to the end of the dictionary upon access. Thus, an attacker could fill the dictionary with spoofed requests and predictably evict an active, legitimate user's entry (resetting their rate limit counter) via the standard Python insertion-order eviction.
**Learning:** Default Python dictionary iteration order is strictly insertion-ordered, not access-ordered. Simple eviction based on this without updating access order means you are evicting the oldest *inserted* item, not the least recently *used* item.
**Prevention:** When implementing a strict LRU eviction policy using Python dictionaries, always move existing keys to the end of the dictionary upon access by explicitly popping and re-inserting them (e.g., `val = d.pop(key); d[key] = val`).
