...
## 2026-05-16 - [Rate Limit Bypass via LRU Cache Eviction Flaw]
**Vulnerability:** When implementing memory-bound security state structures (like dictionary-based rate limiters with `MAX_TRACKED_IPS`), relying solely on default Python dictionary insertion-order for eviction (`next(iter(dictionary))`) is insufficient. Active legitimate IPs can be inserted first, and when an attacker floods the dictionary with dummy or spoofed IPs, the oldest inserted (the legitimate IP) is evicted. This wipes their rate limit history and allows them to bypass limits completely upon their next request, effectively turning a DoS attack against the tracking structure into a rate limit bypass for active users.
**Learning:** Default Python dictionaries maintain insertion order, not access order. Evicting `next(iter(dictionary))` removes the oldest inserted item, which is not necessarily the Least Recently Used (LRU) item.
**Prevention:** Implement a strict Least Recently Used (LRU) eviction policy for stateful security structures. Move keys to the end of the dictionary upon access (e.g., `val = d.pop(key); d[key] = val`) before evicting the oldest via `next(iter(dictionary))`.
