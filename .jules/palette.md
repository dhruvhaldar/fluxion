...
## 2024-05-15 - Robust Screen Reader Focus for Permalinks
**Learning:** When deep-linking to internal page sections (e.g., using heading anchors), relying solely on the URL hash is often insufficient for robust screen reader focus management. The target element must be explicitly focusable so that the screen reader correctly begins reading from the linked content rather than the top of the page.
**Action:** Always add `tabindex="-1"` to internal jump-link target elements (like `<h2 id="...">`). Pair this with CSS `:focus { outline: none; }` to maintain visual polish by suppressing the browser's default focus ring when the element receives programmatic focus via deep-linking. Ensure the jump links themselves provide clear visual context for sighted users (e.g. via `title="Permalink to this section"`) to complement `aria-label` attributes for screen readers.
