...
def rate_limit():
    ...
    if request.method not in allowed_methods:
        ...
        return "Method Not Allowed", 405, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
    ...
    if (request.content_length is not None and request.content_length > 0) or 'Transfer-Encoding' in request.headers:
        log_early_block(f"unexpected_body_{ip}", f"Security Event: Blocked request from {repr(safe_ip)} due to unexpected request body in read-only API.")
        return "Payload Too Large", 413, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
...
def index():
    ...
    <body id="top" tabindex="-1">
        ...
        <main id="main" tabindex="-1">
            ...
            <h2 id="lid-driven-cavity-streamlines" tabindex="-1">Lid Driven Cavity Streamlines (<abbr title="Reynolds number" tabindex="0">Re</abbr>=100) <a href="#lid-driven-cavity-streamlines" class="heading-anchor" aria-label="Permalink to Lid Driven Cavity Streamlines (Re=100)" title="Permalink to Lid Driven Cavity Streamlines (Re=100)"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg></a></h2>
            <figure>
            ...
            <h2 id="grid-convergence-study" tabindex="-1">Grid Convergence Study <a href="#grid-convergence-study" class="heading-anchor" aria-label="Permalink to Grid Convergence Study" title="Permalink to Grid Convergence Study"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg></a></h2>
            <figure>
            ...
            <h2 id="convection-scheme-comparison" tabindex="-1">Convection Scheme Comparison <a href="#convection-scheme-comparison" class="heading-anchor" aria-label="Permalink to Convection Scheme Comparison" title="Permalink to Convection Scheme Comparison"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg></a></h2>
            <figure>
...
def send_assets(path):
    ...
    if '..' in path or path.startswith('/') or '%' in path:
        log_early_block(f"dir_traversal_{ip_key}", f"Security Event: Blocked request from {repr(safe_ip)} due to potential directory traversal. path: {repr(path)}")
        return "Bad Request", 400, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
    ...
    if path.startswith('.') or '/.' in path:
        log_early_block(f"hidden_file_{ip_key}", f"Security Event: Blocked request from {repr(safe_ip)} for hidden file/directory. path: {repr(path)}")
        return "Bad Request", 400, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
    ...
    if not re.fullmatch(r'^[a-zA-Z0-9_./-]+\Z', path):
        log_early_block(f"invalid_chars_{ip_key}", f"Security Event: Blocked request from {repr(safe_ip)} due to invalid characters in path. path: {repr(path)}")
        return "Bad Request", 400, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
    ...
    if ext.lower() not in allowed_extensions:
        log_early_block(f"unsupported_media_{ip_key}", f"Security Event: Blocked request from {repr(safe_ip)} due to unsupported media type. ext: {repr(ext)}")
    ...
    if not requested_path.startswith(assets_dir + os.sep):
        log_early_block(f"out_of_bounds_{ip_key}", f"Security Event: Blocked request from {repr(safe_ip)} due to out-of-bounds resolved path. path: {repr(path)}")
        return "Bad Request", 400, {"Content-Type": "text/plain; charset=utf-8", "Connection": "close"}
