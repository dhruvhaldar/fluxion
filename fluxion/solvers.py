...
class LinearSolver:
    ...
    def solve_sor(p, rhs, grid, omega=1.7, max_iter=5000, tol=1e-5):
        ...
        # ⚡ Bolt: Replace np.putmask with standard NumPy strided slicing and inline vector math
        # to cleanly evaluate and assign updates only on required sub-grids, avoiding masked assignment overhead.
        for it in range(max_iter):
            if it > 0 and it % check_interval == 0: np.copyto(p_old, p_new)

            # Red update
            p_new[1:-1:2, 1:-1:2] = mult_x * ((p_new[1:-1:2, 2::2] + p_new[1:-1:2, :-2:2]) * mult_y_over_x + p_new[2::2, 1:-1:2] + p_new[:-2:2, 1:-1:2] - rhs_eff[0::2, 0::2]) + one_minus_omega * p_new[1:-1:2, 1:-1:2]
            p_new[2:-1:2, 2:-1:2] = mult_x * ((p_new[2:-1:2, 3::2] + p_new[2:-1:2, 1:-2:2]) * mult_y_over_x + p_new[3::2, 2:-1:2] + p_new[1:-2:2, 2:-1:2] - rhs_eff[1::2, 1::2]) + one_minus_omega * p_new[2:-1:2, 2:-1:2]

            # Black update
            p_new[1:-1:2, 2:-1:2] = mult_x * ((p_new[1:-1:2, 3::2] + p_new[1:-1:2, 1:-2:2]) * mult_y_over_x + p_new[2::2, 2:-1:2] + p_new[:-2:2, 2:-1:2] - rhs_eff[0::2, 1::2]) + one_minus_omega * p_new[1:-1:2, 2:-1:2]
            p_new[2:-1:2, 1:-1:2] = mult_x * ((p_new[2:-1:2, 2::2] + p_new[2:-1:2, :-2:2]) * mult_y_over_x + p_new[3::2, 1:-1:2] + p_new[1:-2:2, 1:-1:2] - rhs_eff[1::2, 0::2]) + one_minus_omega * p_new[2:-1:2, 1:-1:2]
