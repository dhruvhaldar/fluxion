...
class LinearSolver:
    ...
    def solve_sor(p, rhs, grid, omega=1.7, max_iter=5000, tol=1e-5):
        ...
        # ⚡ Bolt: Pre-compute slice objects to avoid creating them inside the loop
        # We replace np.putmask which calculates the equation across the full grid and
        # discards half, with explicit strided slicing (e.g. 0::2) that directly targets subgrids.
        red_slices = [(slice(0, None, 2), slice(0, None, 2)), (slice(1, None, 2), slice(1, None, 2))]
        black_slices = [(slice(0, None, 2), slice(1, None, 2)), (slice(1, None, 2), slice(0, None, 2))]
        ...
        if mult_y_over_x == 1.0 and omega == 1.0:
            for it in range(max_iter):
                ...
                for sl_i, sl_j in red_slices:
                    p_slice[sl_i, sl_j] = (p_right[sl_i, sl_j] + p_left[sl_i, sl_j] + p_up[sl_i, sl_j] + p_down[sl_i, sl_j] - rhs_eff[sl_i, sl_j]) * mult_x

                for sl_i, sl_j in black_slices:
                    p_slice[sl_i, sl_j] = (p_right[sl_i, sl_j] + p_left[sl_i, sl_j] + p_up[sl_i, sl_j] + p_down[sl_i, sl_j] - rhs_eff[sl_i, sl_j]) * mult_x
            ...
            for it in range(max_iter):
                ...
                for sl_i, sl_j in red_slices:
                    p_slice[sl_i, sl_j] = (p_right[sl_i, sl_j] + p_left[sl_i, sl_j] + p_up[sl_i, sl_j] + p_down[sl_i, sl_j] - rhs_eff[sl_i, sl_j]) * mult_x + p_slice[sl_i, sl_j] * one_minus_omega
            ...
            for it in range(max_iter):
                ...
                for sl_i, sl_j in red_slices:
                    p_slice[sl_i, sl_j] = ((p_right[sl_i, sl_j] + p_left[sl_i, sl_j]) * mult_y_over_x + p_up[sl_i, sl_j] + p_down[sl_i, sl_j] - rhs_eff[sl_i, sl_j]) * mult_x
            ...
            for it in range(max_iter):
                ...
                for sl_i, sl_j in red_slices:
                    p_slice[sl_i, sl_j] = ((p_right[sl_i, sl_j] + p_left[sl_i, sl_j]) * mult_y_over_x + p_up[sl_i, sl_j] + p_down[sl_i, sl_j] - rhs_eff[sl_i, sl_j]) * mult_x + p_slice[sl_i, sl_j] * one_minus_omega
